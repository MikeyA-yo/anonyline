...
export async function CreateAcct(formState: any, formData: FormData) {
  try {
    ...
    const emailValidationResponse = await fetch(
      `https://emailvalidation.abstractapi.com/v1/?api_key=${process.env.ABSTRACT_API}&email=${email}`
    );
    const emailValidationResult = await emailValidationResponse.json();
    if (!emailValidationResult.is_valid_format.value) {
      return {
    ...
    if (res) {
      return {
        success:
          ...
          email +
          " for verification, if you don't see it check spam folder",
      };
    }
  } catch (e) {
